...
void init(){
    ...
    for(int i = 0; i < ENVIRONMENT_SIZE; i++){
        for(int j = 0; j < ENVIRONMENT_SIZE; j++){
            grow[i][j] = 0;
            c[i][j] = 1.0 - (i*1.0)/(ENVIRONMENT_SIZE-1);
        }
    }
...
void sor(){
    ...
    for(int i = 0; i < ENVIRONMENT_SIZE; i++){
        for(int j = 0; j < ENVIRONMENT_SIZE; j++){
            ...
            if(i == ENVIRONMENT_SIZE - 1) c[i][j] = 0;
        }
    }
    ...
    }while(delta > TOLERANCE);//if error did not be update so it's (initial) zero and we can stop iteration (tolerance can be define above)
}
...
void growth(){
    Virus currentCandidate[ENVIRONMENT_SIZE*ENVIRONMENT_SIZE];//storing currently candidate because each time addVirus() is called...
                                                              //... a candidate will be remove from candidate[] making change in indices
    for(int i = 0; i < nCandidate; i++) {
    ...
    //for each candidate, we draw a random number between 0 and 1 to compare with it's probability. If smaller, turn it into a virus
    for(int i = 0; i < tmp; i++){
...
int main(){
    ...
    //writing the last matrix of nutrient concentration (the environment) to a file "output.txt" (for using on visualizing in MATLAB)
